 ...
 */
	...
	revFunc.revolution = function(options) {
		 ...
		 * pause method
		 */
		revFunc.revpause= function(options) {
		 ...
		 * resume method
		 */
		revFunc.revresume = function(options) {
					return this.each(function() {
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0) {
		 ...
		 * next method
		 */
		revFunc.revnext = function(options) {
					return this.each(function() {
		 ...
		 * resume method
		 */
		revFunc.revprev = function(options) {
					return this.each(function() {
						// CATCH THE CONTAINER
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0)
		...
		/**
		 * get max slide method
		 */
		revFunc.revmaxslide = function(options) {
				// CATCH THE CONTAINER
		...
		/**
		 * get current slide method
		 */
		revFunc.revcurrentslide = function(options) {
						// CATCH THE CONTAINER
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0) {
							var bt = container.parent().find('.tp-bannertimer');
		 ...
		 * last slide method
		 */
		revFunc.revlastslide = function(options) {
				// CATCH THE CONTAINER
				var container=jQuery(this);
				if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0) {
					var bt = container.parent().find('.tp-bannertimer');
		 ...
		 * show slide method
		 */
		revFunc.revshowslide = function(slide) {
					return this.each(function() {
						// CATCH THE CONTAINER
						var container=jQuery(this);
						if (container!=undefined && container.length>0 && jQuery('body').find('#'+container.attr('id')).length>0) {
				...
				try{
					...
					container.parent().swipe( {
						...
						tap:function(event, target) {
							...
							if(container.hasClass("hovered") == false){
								...
								if (handlecaption==0) {
									...
									if (nextcaption.hasClass("tp-videolayer") || nextcaption.find('iframe').length>0 || nextcaption.find('video').length>0 ) {
										...
										if (vimeoid!=undefined && String(vimeoid).length>1 && nextcaption.find('iframe').length==0) {
											...
											nextcaption.append('<iframe style="visible:hidden" src="'+httpprefix+'://player.vimeo.com/video/'+vimeoid+'?'+vida+'" width="'+vidw+'" height="'+vidh+'" style="width:'+vidw+'px;height:'+vidh+'px" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>');
										}
																			...
																			player = new YT.Player(frameID, {
																				events: {
																					...
																					'onReady': function(event) {
																						...
																						var ytcont = jQuery(event.target.getIframe());				
																						var nextcaption = ytcont.closest('.tp-caption');
																						var videorate = nextcaption.data('videorate');
																						var videostart = nextcaption.data('videostart');
															...
															if (nextcaption.data('autoplay')==true || autoplaywason==true) {
																...
																html5vid.data('timerplay',setTimeout(function() {
																	...
																	if (nextcaption.data('volume')=="mute")
																		...
																		//set autoplay only once
																		//stop autoplay next time
																		if (nextcaption.data('autoplayonlyfirsttime') == true || nextcaption.data('autoplayonlyfirsttime')=="true" ) {
																			nextcaption.data('autoplay',false);
																			nextcaption.data('autoplayonlyfirsttime',false);
																		}
